...
def test_fun():
    logging.info(" basic thread function")
...
if __name__ =="__main__":
    ...
    for i in range(context.num_workers):
        threads.append(Thread(name = str(i),target=test_fun))
        threads[-1].start()
    ...
    t = []
    for i in range(context.num_workers):
        t.append(Thread(name = str(i),target=crawl_urls, args=(context,), daemon=True))
        t[-1].start()
    # for s in t:
    #     s.join()
    context.urls_to_visit.join()

    toc = time.perf_counter()

    logging.info(f"Time taken to crawl= {toc - tic:0.4f} seconds")
    logging.info(f"Total URLs crawled = {len(context.visited_urls)}")
    logging.info("Finished Crawling")
    logging.debug(f"URLs Crawled = {context.crawl_results.keys()}")
    # logging.debug(f"Results of Crawl = {context.crawl_results}")
